...
namespace feature {
    ...
    void HistogramFeatureExtractor::setHistogramBinSize(const float& histogram_bin_size)
    {
     ...
     */
    void HistogramFeatureExtractor::compute(PointICloudConstPtr cloud_in, Feature* feature)
    {
        ...
        for (int i = 0u; i < pt_num; ++i) {
            ...
            stat_feat[static_cast<int>(xsize + ysize + (pt.z - min_pt_.z) / zstep)]++;
        }
        // update feature
        for (size_t i = 0u; i < xsize; ++i) {
            ...
            (*feature).push_back(
                    FeatureElement(
                            feature_name, static_cast<float>(stat_feat[i]) / static_cast<float>(pt_num)
                    )
            );
        }
    ...
    void HistogramFeatureExtractor::getMinMaxCenter(PointICloudConstPtr cloud_in)
    {
        float xsum = 0.0;
        float ysum = 0.0;
        float zsum = 0.0;
