...
namespace feature {
    ...
    void EigenValueFeatureExtractor::compute(const std::vector<PointICloudPtr>& cloud_clusters,
                                             std::vector<Feature>* features)
    {
        common::Clock clock;
        if (features == nullptr) {
            ROS_ERROR("EigenValueFeatureExtractor::compute(), [features] should not be null.");
            return;
        }
        (*features).clear();
        ...
        ROS_INFO_STREAM("Eigenvalue-based features extracted. Took " << clock.takeRealTime() << "ms.");
                                             ...
                                             Feature* feature)
    {
        ...
        /*
         * <2>Find the covariance matrix. Since it is symmetric, we only bother with the upper diagonal.
         * |                                |
         ...
         *  where P is the set of k nearest neighbors and p = med(pi \in P) is its medoid.
         */
        ...
        //<5>Eigenvalues are normalised to sum up to 1, so as to increase robustness against changes in point density.
         ...
         * @note refer to 《FAST SEMANTIC SEGMENTATION OF 3D POINT CLOUDS WITH STRONGLY VARYING DENSITY》
         */
        ...
        ///TODO number of points is important, seems that different segments with different size
        (*feature).push_back(FeatureElement("Num", num_points));

        (*feature).push_back(FeatureElement("Linearity", (e1 - e2) / e1 / kLinearityMax));
        (*feature).push_back(FeatureElement("Planarity", (e2 - e3) / e1 / kPlanarityMax));
        (*feature).push_back(FeatureElement("Sphericity", e3 / e1 / kScatteringMax));
        (*feature).push_back(
                FeatureElement("Omnivariance", std::pow(e1 * e2 * e3, kOneThird) / kOmnivarianceMax));
        (*feature).push_back(FeatureElement("Anisotropy", (e1 - e3) / e1 / kAnisotropyMax));
        (*feature).push_back(FeatureElement("Eigenentropy",
                                                  (e1 * std::log(e1)) + (e2 * std::log(e2)) +
                                                  (e3 * std::log(e3)) / kEigenEntropyMax));
        (*feature).push_back(
                FeatureElement("SurfaceVariation", e3 / sum_of_eigenvalues / kChangeOfCurvatureMax));
