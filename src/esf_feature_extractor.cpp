...
namespace feature {
    ...
    void ESFFeatureExtractor::compute(const std::vector<PointICloudPtr>& cloud_clusters,
                                      std::vector<Feature>* features)
    {
        common::Clock clock;
        if (features == nullptr) {
            ROS_ERROR("ESFFeatureExtractor::compute(), [features] should not be null.");
            return;
        }
        (*features).clear();
        ...
        ROS_INFO_STREAM("Ensemble of Shapes Function features extracted. Took " << clock.takeRealTime() << "ms.");
    }

    void ESFFeatureExtractor::compute(const PointICloud& cloud_in, Feature* feature)
    {
        if (feature == nullptr) {
            return;
        }
        ...
        // After estimating the ensemble of shape functions, the esf640 should be of size 1
        if (esf640->size() != 1u) {
        ...
        for (unsigned int i = 0u; i < kESFDimension; ++i) {
            (*feature).push_back(
                    FeatureElement("esf_" + std::to_string(i), esf640->points[0].histogram[i]));
        }
        if ((*feature).size() != kESFDimension) {
